<?php
/*
 * This file is part of the LINGLemon/ExpressService.
 *
 * (c) LINGLemon <396955213@qq.com>
 *
 ...
 * Express
 *
 * @author    LINGLemon <396955213@qq.com>
 * @copyright 2018 LINGLemon <396955213@qq.com>
 *
 * @link      https://github.com/LINGLemon/ExpressService
 */

namespace app\common\services\express;

use Yii;

class ExpressService extends \yii\base\Component
{
    /**
     * 快递100查询地址
     *
     ...
     * @var array
     */
    public static $logisticsCompanyData = array(
        'shunfeng' => '顺丰',
        'yuantong' => '圆通速递',
        'shentong' => '申通',
        'yunda' => '韵达快运',
    ...
    public static function search($number, $code = '', $output = 'json')
    {
        ...
        $result = $output == 'json' ? $result : json_decode($result, true);
        if (!isset($result['status'])) {
            $result['status'] = 200;
        }
        if (!isset($result['com'])) {
            $result['status'] = 400;
            $result['message'] = '快递100服务器错误，请稍后重试！';
        }
        return $result;
    }
    ...
    public static function getCode($number, $output = 'name')
    {
        ...
        return $output == 'name' ? (isset(self::$logisticsCompanyData[$code]) ? self::$logisticsCompanyData[$code] : $code) : $code;
    }
    ...
    {
        return self::$logisticsCompanyData;
    }
